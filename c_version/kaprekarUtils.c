...
int isKaprekar(int n)
{
  if(n < 1)
    {
      return 0; //changed "false" to "0", since Boolean types don't exist in C
    }

int i;
long square = n * (long) n;
int numberOfDigits = (int) log10(square) + 1; //change name to numberOfDigits, also changed log10 to operate on square
long modulus = 1; //change to 1, not 0
long first, second;

for(i=1; i <= numberOfDigits; i++)
  {
  modulus *= 10; //fix "modulous" to "modulus"
  first = square / modulus;
  second = square % modulus;
    if(second > 0 && first + second == n) //Put these on the same line
      {
        return 1;
      }
  }
